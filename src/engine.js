...
- messagesやpostsには、実画像の代わりに「[添付: どんな画像か]」という擬似添付を入れてよい(textフィールドの中にそのまま書く)。例: 「[添付: カラオケから出てくる蓮と美桜らしき二人の、遠くてブレた写真]」「これ見て…[添付: 寝てる蓮の横顔の自撮り]」。証拠写真・自撮り・スクショの誤爆・盗撮など、その場面で自然なときだけ使い、多用はせずここぞで効かせる。画像が情報を伝える経路になる場合も、誰がどう撮った/送ったかの筋は通すこと。
...
export async function npcReplies(S, threadId, playerText) {
  ...
  const tail =
    ...
    `自然なときは実画像の代わりに「[添付: どんな画像か]」という擬似添付をtextに入れてよい(多用しない)。\n` +
